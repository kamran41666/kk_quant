...
def test_expired_lease_can_be_recovered_without_duplicate_job(db_session):
    ...
    first = lease_next_job(db_session, worker_id="worker-a", now=datetime(2024, 1, 5, 0, 0, tzinfo=timezone.utc), lease_seconds=10)
    assert first is not None and first.id == jobs[0].id
    assert heartbeat_job(db_session, first.id, worker_id="worker-a", now="2024-01-05T00:00:05+00:00", lease_seconds=20).status == "running"
    ...
    assert fail_job(db_session, second.id, worker_id="worker-b", error="temporary", retry=True).status == "pending"
    third = lease_next_job(db_session, worker_id="worker-c", now="2024-01-05T00:01:00+00:00", lease_seconds=30)
    assert third is not None
    completed = complete_job(db_session, third.id, worker_id="worker-c", result={"ok": True})
...
def test_blocked_job_requires_reason_and_cannot_be_claimed_again(db_session):
    ...
    blocked = block_job(db_session, job.id, worker_id="worker-a", reason="manual_valuation_required")
