...
def test_release_requires_all_gates_and_is_not_legacy_inherited(db_session):
    release = create_strategy_release(
        ...
        strategy_fingerprint="2" * 64, research_evidence={"source": "new-bundle"},
        execution_policy={"auto_submit": False}, risk_policy={"max_drawdown": "0.2"},
    )
    with pytest.raises(PromotionError, match="gates_failed"):
    ...
    assert db_session.get(type(release), release.id).status == "portfolio_passed"
