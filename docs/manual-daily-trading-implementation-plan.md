...
- M8终结报告新增经验证交易日历参数，终结时严格比较pilot启动日之后的连续30个交易日序列；缺日、跨日补齐或日历不可用均不能生成通过状态。人工API在终结前重新检查日历覆盖，并正确处理`datetime`输入的日期部分。
- P0/P1、日亏损次数和最大回撤证据新增显式整数/有限数值校验；格式错误以`ManualPilotError`失败关闭，不产生未处理异常。
- 验证：`tests/test_manual_pilot.py`为`4 passed`，相关文件`compileall`通过。该工程验证不代表已完成真实30日观察；M9仍没有券商认证、订单发送或真实资金路径。

## 28. 2026-09-10远端建设审查与H1加固记录

- 对`5c225a4...fb92672`的日频闭环新增建设完成协议、研究、账本、API、调度和前端审查；完整发现、复现和遗留项见[`日频建设审查`](research-runs/manual-daily-implementation-audit-2026-09-10.md)。
...
## 32. H2b cohort公司行动子账检查点

- 新增不可变`ResearchCorporateAction`和normalized action表适配器；action ID由规范化经济字段稳定hash生成，source文件hash进入action hash。同ID相同内容和同经济事件重复输入幂等，不同来源/内容冲突直接拒绝。
- 登记日收盘按cohort冻结权益，登记后卖出不删除资格。除权日现金进入独立应收，派息日才转入现金；红股先按账户总量取整，再按小数余数降序、cohort ID升序分配，确保cohort合计严格等于账户新增股数。
