  ...
  <div class="page">
    ...
    <section class="card compare-card" aria-labelledby="compare-title">
      <div class="compare-heading">
        <div>
          <h2 id="compare-title">回测对比</h2>
          <p class="hint">并排查看收益、风险和数据证据；对比结果仅用于研究，不代表未来收益。</p>
        </div>
...
function costScenarioLabel(market?: string, id?: string, model?: any, fundFeeRate?: unknown): string {
  ...
  return ({
    paper_baseline_v1: '纸面基线',
    paper_low_impact_v1: '纸面低冲击',
    paper_high_impact_v1: '纸面高冲击',
  } as Record<string, string>)[id || ''] || (id ? String(id) : '未记录')
}
...
function evidenceLabel(item: any): string {
  ...
  if (datasetHashes.length > 1 && item?.evidence?.data_content_hash) {
    return `NAV ${datasetHashes.length} 只 · ${String(item.evidence.data_content_hash).slice(0, 8)}`
  }
...
@media (max-width: 720px) { .compare-controls { align-items: stretch; flex-direction: column; } .compare-button { width: 100%; } }
</style>
