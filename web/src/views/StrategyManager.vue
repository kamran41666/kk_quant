  ...
  <div class="page">
    ...
    <section v-if="showForm" class="card form-card" aria-labelledby="create-title">
      <div class="form-title-row">
        <div><h2 id="create-title">按协议新建策略</h2><p class="hint">保存后可直接在回测中心选择；类路径必须位于本地 strategies 包。</p></div>
        <span class="protocol-badge">协议 {{ protocol.version || '1.0' }}</span>
      </div>
      <div class="form-grid">
        <label>策略名称<input v-model.trim="form.name" placeholder="均线动量策略" /></label>
        <label>策略类路径<input v-model.trim="form.strategy_class" placeholder="strategies.my_strategy.MomentumStrategy" /></label>
        <label>适用市场<select v-model="form.market"><option value="a-share">A 股</option><option value="cn-fund">国内基金</option><option value="us-equity">美股（回测暂未开放）</option></select></label>
        <label>策略说明<textarea v-model.trim="form.description" rows="2" placeholder="说明信号、持仓和调仓逻辑"></textarea></label>
        <label class="wide-field">参数（JSON 对象）<textarea v-model="form.paramsStr" rows="4" spellcheck="false" placeholder='{"lookback": 20, "top_n": 5}'></textarea><span class="field-hint">策略在 initialize 中通过 self._strategy_kwargs 读取；不能放入账户密钥或网络配置。</span></label>
      </div>
      <div class="template-row"><span>快速填入模板</span><button v-for="preset in parameterPresets" :key="preset.label" class="preset-button" type="button" @click="form.paramsStr = preset.value">{{ preset.label }}</button></div>
...
const form = ref({ name: '', strategy_class: '', description: '', market: 'a-share' as 'a-share' | 'cn-fund' | 'us-equity', paramsStr: '{}' })
...
async function createStrategy() {
  ...
  try { params = JSON.parse(form.value.paramsStr); if (!params || Array.isArray(params) || typeof params !== 'object') throw new Error('参数必须是 JSON 对象') }
  catch (e: any) { error.value = '参数格式错误：' + e.message; return }
  if (!form.value.name || !form.value.strategy_class) { error.value = '请填写策略名称和类路径'; return }
  try { saving.value = true; error.value = ''; await api.post('/strategies', { name: form.value.name, strategy_class: form.value.strategy_class, description: form.value.description, market: form.value.market, params }); showForm.value = false; form.value = { name: '', strategy_class: '', description: '', market: 'a-share', paramsStr: '{}' }; await loadStrategies() }
...
function isProtocolCompatible(strategy: Strategy) { return strategy.strategy_class.startsWith('strategies.') && Boolean(strategy.protocol_version || protocol.value.version) }
onMounted(() => { loadStrategies(); loadProtocol() })
</script>
